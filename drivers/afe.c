...
// read back conversions from one device
// todo: is this needed if we're going to be doing VIN and AUX conversions at once?
// we must do at least 6 conversions anyways each time we initiate a conversion
uint8_t afe_read_conversion(struct AFEConfig *afe, uint8_t devaddr, uint8_t regaddr) {
  ...
  // prv_transfer_32_bits(afe, 0xC382865A);
  prv_write(afe, devaddr, AFE_READ, false, regaddr);

  // turn off read operation on all parts
  // prv_transfer_32_bits(afe, 0x01B617EA);
  prv_write(afe, AFE_DEVADDR_MASTER, AFE_CONTROL_HB, true,
            CTRL_HB_CONV_RSLT_READ_ALL | CTRL_HB_CONV_RSLT_READ_DSBL);

  // set control register of the part we are reading back from
  prv_write(afe, devaddr, AFE_CONTROL_HB, false,
            CTRL_HB_CONV_RSLT_READ_ALL | CTRL_HB_CONV_INPUT_ALL);

  // allow conversions to be initiated using CNVST pin on selected part
  prv_write(afe, devaddr, AFE_CNVST_CONTROL, false, CNVST_IN_PULSE);

  // initiate conversions through falling edge
  io_set_state(afe->cnvst, IO_LOW);
  __delay_cycles(50000);
  io_set_state(afe->cnvst, IO_HIGH);

  // gate CNVST signal on all parts
  prv_write(afe, AFE_DEVADDR_MASTER, AFE_CNVST_CONTROL, true,
            CNVST_IN_GATED);
  ...
  if ((reply >> 27) != devaddr || ((reply >> 23) & 0xF) != devaddr) {
    // todo: consider returning results to a pointer
    return 0;
  }

  // conversion data are 12 bits (D22:D11)
  return (reply >> 11) & 0xFFF;
}
