...
static bool prv_crc8_check(struct AFEConfig *afe, uint32_t val) {
  uint8_t crc = afe_crc8_calculate(afe, val >> 10);
...
bool afe_init(struct AFEConfig *afe) {
  ...
  // 0x01C2B6E2
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_CONTROL_LB, true, CTRL_LB_DSY_CHN_RDBCK_ENBL | CTRL_LB_LOCK_DEVICE_ADDR);

  // 0x038716CA
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_READ, true, READ_CONTROL_LB);

  // 0xF800030A
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_CELL_VOLTAGE_1, false, READ_CELL_VOLTAGE_1);
...
uint8_t afe_crc8_calculate(struct AFEConfig *afe, uint32_t val) {
...
uint32_t afe_write(struct AFEConfig *afe, uint16_t device_addr, uint16_t register_addr, bool all, uint16_t data) {
  ...
  msg |= (afe_crc8_calculate(afe, msg >> 11) << 3) | AD7280A_WRITE_BIT_PATTERN;

  return prv_transfer_32_bits(afe, msg);
}


uint8_t afe_read_conversions(struct AFEConfig *afe, uint16_t device_addr, uint16_t register_addr) {
...
bool afe_set_cbx(struct AFEConfig *afe, uint16_t device_addr, uint8_t cells) {
  afe_write(afe, device_addr, AD7280A_CELL_BALANCE, false, cells);
...
bool afe_set_cbx_timer(struct AFEConfig *afe, uint16_t device_addr, uint8_t cbx_timer, uint8_t duration) {
  afe_write(afe, device_addr, cbx_timer, false, duration);

  return true;
}


bool afe_set_charge_thres(struct AFEConfig *afe, uint16_t device_addr, uint8_t val) {
  // write undervoltage threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_CELL_UNDERVOLTAGE, true, (afe->charge)->voltage_low);

  // write overvoltage threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_CELL_OVERVOLTAGE, true, (afe->charge)->voltage_high);

  // set adc under-temperature threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_AUX_ADC_UNDERVOLTAGE, true, (afe->charge)->temp_low);

  // set adc over-temperature threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_AUX_ADC_OVERVOLTAGE, true, (afe->charge)->temp_high);
  return true;
}


bool afe_set_discharge_thres(struct AFEConfig *afe, uint16_t device_addr, uint8_t val) {
  // write undervoltage threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_CELL_UNDERVOLTAGE, true, (afe->discharge)->voltage_low);

  // write overvoltage threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_CELL_OVERVOLTAGE, true, (afe->discharge)->voltage_high);

  // set adc under-temperature threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_AUX_ADC_UNDERVOLTAGE, true, (afe->discharge)->temp_low);

  // set adc over-temperature threshold
  afe_write(afe, AD7280A_DEVICEADDR_ALL, AD7280A_AUX_ADC_OVERVOLTAGE, true, (afe->discharge)->temp_high);
  return true;
}
...
void afe_self_test(struct AFEConfig *afe) {
  ...
  // 0x01B81092
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_CONTROL_HB, true, CTRL_HB_CONV_IN_SELF_TEST);

  // enable the daisy-chain register read operation on all parts
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_CONTROL_LB, true, CTRL_LB_DSY_CHN_RDBCK_ENBL | CTRL_LB_LOCK_DEVICE_ADDR);

  // write the self-test conversion register to the read registers
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_READ, true, READ_SELF_TEST);

  // program the CNVST control register, to allow a single CNVST pulse
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_CNVST_CONTROL, true, CNVST_IN_PULSE);
  ...
  // gate CNVST
  afe_write(afe, AD7280A_DEVICEADDR_MASTER, AD7280A_CNVST_CONTROL, false, CNVST_IN_GATED);
