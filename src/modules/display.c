...
void display_update(const struct DisplayConfig *display) {
  if (controls_faulted()) {
    snprintf(display_buffer[1], 20, "Battery fault");
  ...
  snprintf(display_buffer[0], 20, "S: %d RPM: %.3f %s", speed_convert(&velocity), mc_state_value(display->mc_state, MC_LEFT, MC_RPM), cruise);
...
//  snprintf(display_buffer[2], 20, "Target: %3d %3s", (int)controls_cruise_target(), unit);
  snprintf(display_buffer[3], 20, "V: %.2f A: %.2f %d", mc_state_value(display->mc_state, MC_LEFT, MC_VOLTAGE), mc_state_value(display->mc_state, MC_LEFT, MC_CURRENT), controls_cruise_target());
